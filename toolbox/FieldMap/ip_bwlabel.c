                 ...
                 const mxArray   *prhs[])   /* Input arguments. */
{
   ...
   if (nrhs == 0) mexErrMsgTxt("usage: [L,NUM]=ip_bwlabel(BW,CONN)");
   if (nrhs != 2) mexErrMsgTxt("ip_bwlabel: 2 input arguments required");
   if (nlhs != 2) mexErrMsgTxt("ip_bwlabel: 2 output argument required");

   /* Get binary map. */

   if (!mxIsNumeric(prhs[0]) || mxIsComplex(prhs[0]) || mxIsSparse(prhs[0]) || !mxIsDouble(prhs[0]))
   {
      mexErrMsgTxt("ip_bwlabel: BW must be numeric, real, full and double");
   }
   ndim = mxGetNumberOfDimensions(prhs[0]);
   if ((ndim < 2) || (ndim > 3))
   {
      mexErrMsgTxt("ip_bwlabel: BW must be 2 or 3-dimensional");
   }
   ...
   {
      mexErrMsgTxt("ip_bwlabel: CONN must be numeric, real, full and double");
   }
   tmp1 = mxGetM(prhs[1]);
   tmp2 = mxGetN(prhs[1]);
   ...
   {
      mexErrMsgTxt("ip_bwlabel: CONN must be 6, 18 or 26");
   }
